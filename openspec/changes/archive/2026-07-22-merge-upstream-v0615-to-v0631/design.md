...
### D7: 功能增强按"适配接入（主体不变）"处理，文件预览拆组件

**选择**：ChatInput/ChatPanel 虽被深度定制，但我们**欢迎新功能**，采用"适配接入"而非整文件合：
- MoA（`3decc0e9`）、推理滑块（`e683cd36`）、消息引用（`d246455e`）为加法型增量，在现有主体上挂控件/状态即可，不改结构主体。
- 文件预览（`62166315`）**只摘独立预览组件**挂到现有 `MessageItem.vue`，**不吃**上游对 `MessageItem.vue`（−469）/`MarkdownRenderer.vue`（−128）的渲染层重构，也不引入 `ToolChangeCard` 拆分。
- 文件预览依赖 workspace-diff 数据链，**前置补 `#1919`** 的 `workspace-run-changes-store.ts` 建表，否则预览无数据。

**理由**：符合"主体不发生大变动、但接纳新功能"的原则。前三个功能本就是加法型，适配成本低；文件预览被上游与一次 MessageItem 重构打包在一起，整体合入会大改我们的消息渲染主体，摘组件可保住主体，代价是自行接线。

**非目标**：不接受上游 `MessageItem.vue`/`GroupChatPanel.vue` 的结构重构；工作流相关（`#2073` 等）明确不合。
