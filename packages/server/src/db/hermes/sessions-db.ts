...
export async function listSessionSummaryGroups(
  ...
  try {
    ...
    // Preserve fork per-user session isolation: only include roots owned by the requesting user.
    const userIdFilter = userId != null ? String(userId) : null

    for (const root of idx.byId.values()) {
      if (root.parent_session_id != null) continue
      if (userIdFilter != null && String(root.user_id || '') !== userIdFilter) continue
