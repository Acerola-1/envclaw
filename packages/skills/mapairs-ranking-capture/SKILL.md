...
description: 使用固定 Playwright 脚本按结构化参数生成数智大气浓度排名截图（URL 直连模式）。当任务成果为浓度排名截图、需要城市或站点查询、设置时间类型、主题或截图范围时使用。
---

# 数智大气浓度排名截图（URL 直连）

只执行本 Skill 附带的 `scripts/mapairs_ranking_capture.py`；不要使用 agent-browser、不要自行操作网页、不要修改脚本或猜测页面步骤。脚本会用参数拼接完整 URL 后直接跳转截图，比逐级点击菜单更稳定。
...
将该成果的 `config` JSON 原样写入当前运行目录的 `ranking-config.json`，先定位本 Skill 安装目录再执行：
