...
# 安装后目录结构：<skills>/mapairs-ranking-capture/scripts/本脚本
...
RANKING_PATH = "/dataStatistics/concentrationranking"
...
def read_config(raw: str) -> dict[str, Any]:
    ...
    if config["screenshotScope"] not in SCOPES:
        fail("screenshotScope 仅支持 tableOnly 或 withFilters")
...
def build_ranking_url(config: dict[str, Any]) -> str:
    """按浓度排名页面 URL 约定拼接查询参数。"""
    params: dict[str, Any] = {
    ...
    return build_url(RANKING_PATH, params)
...
def clip_for(page: "Page", scope: str) -> dict | None:
    ...
    if scope == "tableOnly":
        return page.evaluate(
            """() => {
                const a = document.querySelector('.data_list .record');
                const b = document.querySelector('.data_list .table-list');
                if (!a || !b) return null;
                const ra = a.getBoundingClientRect(), rb = b.getBoundingClientRect();
                const x = Math.min(ra.x, rb.x), y = Math.min(ra.y, rb.y);
                const right = Math.max(ra.right, rb.right);
                return { x, y, width: right - x, height: rb.bottom - y };
            }"""
        )
    box = page.locator(".data_list").first.bounding_box()
    return box
...
def capture(config: dict[str, Any], output_dir: Path) -> Path:
    ...
    target_url = build_ranking_url(config)
