#!/usr/bin/env python3
"""Playwright: 浓度排名双截图 - 省级全部 + 城市全部区县 (日累计)"""
...
PROVINCE_MAP = {
    ...
    "南京市": "江苏省", "苏州市": "江苏省", "济南市": "山东省", "青岛市": "山东省",
}
...
def select_cascader_level(page, menu_index, text):
    """选择级联选择器的某一级（点击 .el-checkbox__inner）"""
    node = page.locator(
    ...
    if tables1:
        for table in tables1:
            if len(table) > 1:
                ...
                for row in table[1:]:
                    if len(row) > 2 and city in row[0]:
        ...
        for table in tables2:
            if len(table) > 1:
                ...
                for row in table[1:4]:
                    aqi = row[10] if len(row) > 10 else '-'
                    level = row[11] if len(row) > 11 else '-'
                    lines.append(f"  第{row[0]}名: {row[1]}，综指{row[2]}，AQI {aqi}，等级{level}")
                lines.append("")
...
if __name__ == "__main__":
    print("=" * 60, flush=True)
    print(f"浓度排名双截图 - {CITY_NAME}", flush=True)
    print("=" * 60, flush=True)
    ...
    with sync_playwright() as p:
        ...
        if not province:
            print(f"ERROR: Unknown province for {CITY_NAME}", flush=True)
            browser.close()
            exit(1)

        # 截图1: 省份 + 全部
        print(f"\nStep 2: Screenshot 1 - {province} 全部城市...", flush=True)
        ...
        select_cascader_level(page, 0, province)
        ...
        ss1 = os.path.join(SCREENSHOT_DIR, f"rank_province_{TIMESTAMP}.png")
        cdp_ss(page, context, ss1)
        tables1 = extract_table_data(page)
        print(f"  Tables: {len(tables1)}", flush=True)

        # 截图2: 省份 + 城市 + 全部区县
        print(f"\nStep 3: Screenshot 2 - {CITY_NAME} 全部区县...", flush=True)
        reset_and_navigate(page)
        wait_for_page(page)
        ...
        ss2 = os.path.join(SCREENSHOT_DIR, f"rank_city_{TIMESTAMP}.png")
        cdp_ss(page, context, ss2)
        tables2 = extract_table_data(page)
        print(f"  Tables: {len(tables2)}", flush=True)

        summary = format_summary(tables1, tables2, CITY_NAME, province)
