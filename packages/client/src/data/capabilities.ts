...
export const MAPAIRS_CAPS: CapSkill[] = [
  {
    ...
    params: [
      ...
      { name: '截图类型', type: 'multi', required: false, default: '页面截图' },
    ],
  ...
  },
  {
    ...
    params: [
      { name: '地图模式', type: 'select', required: false, default: '默认' },
      ...
      { name: '缩放等级', type: 'select', required: false, default: '城市层级' },
      { name: '缩放自定义值', type: 'string', required: false, default: '6' },
      { name: '插值图层', type: 'select', required: false, default: '' },
      { name: '风海浪', type: 'select', required: false, default: '开' },
      { name: '左侧面板', type: 'select', required: false, default: '关' },
      { name: '左侧面板区域', type: 'select', required: false, default: '城市' },
      { name: '星地因子', type: 'select', required: false, default: 'PM₂.₅' },
      { name: '星地时间类型', type: 'select', required: false, default: '实时' },
    ],
    outputs: ['一张图截图 (PNG)'],
  ...
  },
  {
    ...
    params: [
      { name: '行政区', type: 'string', required: true, default: '平顶山市' },
      { name: '查询维度', type: 'select', required: false, default: '城市' },
      { name: '乡镇', type: 'select', required: false, default: '全部乡镇' },
